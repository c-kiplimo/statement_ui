...
import { CREATE_ACCOUNT_SHEDULE, GET_ACCOUNT_SCHEDULE_BY_ID, UPDATE_ACCOUNT_SHEDULE } from "@/src/constants/environment"
import { notification } from "antd";
import axios from "axios";

export const createAccountSchedule = async (accountScheduleFormData:SingleAccountsSchedule):Promise<SingleAccountsSchedule>=>{
    const createScheduleUrl = CREATE_ACCOUNT_SHEDULE;

    try {
        const response = await axios
            .post(createScheduleUrl, accountScheduleFormData, {
            headers: {
...
export const fetchAccountSheduleById  = async (id:number):Promise<SingleAccountSchedule>=>{
